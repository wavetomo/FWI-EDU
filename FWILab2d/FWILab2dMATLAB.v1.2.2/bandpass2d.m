function s_filter = bandpass2d(s, dx, dz, kpass, kcut)
%% BANDPASS2D 2D band-pass filter in the wavenumber domain
%
% Inputs:
...
inLowTaper = k < kpass;
win(inLowTaper) = 0.5 * (1 + cos((kpass - k(inLowTaper))/lowTaperLen*pi)); 
inHighTaper = (k >= kpass) & (k <= kcut);
win(inHighTaper) = 0.5 * (1 + cos((k(inHighTaper) - kpass)/highTaperLen*pi));
